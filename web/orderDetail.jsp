...
<div class="panel panel-default" style="margin: 0 auto;width: 95%;">
    ...
    <div class="panel-body">
        <table cellpadding="0" cellspacing="0" align="center" width="100%"
            ...
            <tr>
                <td>送货地址:</td>
                <td>${order.address.detailAddress}</td>
                <td>总价:</td>
                <td>${order.total}</td>
            </tr>
                    ...
                    <table align="center" cellpadding="0" cellspacing="0" width="100%"
                        ...
                        <c:forEach items="${order.items}" var="item" varStatus="i">
                            <tr align="center">
                                <th>${i.count}</th>
                                <th>
                                    <%--<img src="${pageContext.request.contextPath}/${item.product.image}" width="50px" height="50px">--%>
                                    <img src="${item.product.image}" width="50px" height="50px">
                                </th>
                                <th>${item.product.name}</th>
                                <th>${item.product.state}</th>
                                <th>${item.product.upTime}</th>
            ...
            <tr>
                <td align="right" colspan="4" style="margin-right: 40px;">
                    <a href="${pageContext.request.contextPath }/order?method=show"
                       class="btn btn-danger btn-sm">返回订单列表</a>
                    &nbsp;&nbsp;
                    <c:if test="${order.state eq 0}">&nbsp;&nbsp;
                        <button type="button" onclick="payWeiXin('${order.id}','${order.totalFee}')"
